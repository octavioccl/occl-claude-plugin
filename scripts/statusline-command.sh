...
line1+="${sep}"
# Build context window bar (▓ filled, ░ empty)
ctx_bar_width=10
ctx_filled=$(( pct_used * ctx_bar_width / 100 ))
[ "$ctx_filled" -gt "$ctx_bar_width" ] && ctx_filled=$ctx_bar_width
ctx_empty=$(( ctx_bar_width - ctx_filled ))
ctx_bar=""
for i in $(seq 1 "$ctx_filled"); do ctx_bar+="▓"; done
for i in $(seq 1 "$ctx_empty");  do ctx_bar+="░"; done
line1+="${dim}ctx${reset} ${pct_color}${ctx_bar}${reset} ${dim}${used_tokens}/${total_tokens}${reset} ${pct_color}${pct_used}%${reset}"
line1+="${sep}"
